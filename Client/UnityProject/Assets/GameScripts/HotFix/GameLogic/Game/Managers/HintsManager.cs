...
namespace MergeIt.Game.Managers
{
    public class HintsManager : IHintsManager, IInitializable, IDisposable, IUpdatable
    {
        ...
        public void Initialize()
        {
            ...
            _messageBus.AddListener<FindElementMessage>(FindElemMessageHandler);
        }
        ...
        public void Dispose()
        {
            ...
            _messageBus.RemoveListener<FindElementMessage>(FindElemMessageHandler);
        }
        ...
        public void FindElemMessageHandler(FindElementMessage message)
        {
            int id = message.ElementId;
            var findElem = _fieldLogicModel.FieldElements.Values.FirstOrDefault(x => x.ConfigParameters.ElementConfig.Id == id.ToString());
            if(findElem == null) return;
            
            FieldCellComponent cell =
                _fieldLogicModel.CellComponents[findElem.InfoParameters.LogicPosition];
            IFieldElementPresenter candidate = cell.FieldElementPresenter;
            candidate.SetState(FieldElementState.Find);
        }
        ...
        private void FindCandidates()
        {
            ...
            for (int i = 0; i < _randomHeight.Count; i++)
            {
                for (int j = 0; j < _randomWidth.Count; j++)
                {
                    ...
                    if(_fieldLogicModel.IsBeSandedFull(point)) continue;
