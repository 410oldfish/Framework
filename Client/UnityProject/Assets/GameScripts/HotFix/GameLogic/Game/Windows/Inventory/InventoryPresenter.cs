...
namespace MergeIt.Game.Windows.Inventory
{
    public class InventoryPresenter : WindowPresenter<InventoryWindow, InventoryModel>
    {
        ...
        private async UniTaskVoid FillGrid()
        {
            ...
            if (elementsCount > 0)
            {
                for (int i = 0; i < elementsCount; i++)
                {
                    ...
                    InventoryWindowItemComponent cell = await _inventoryFactory.CreateInventoryWindowItem(element);
        ...
        private async void OnBuyCellClick()
        {
            ...
            if (_currencyService.TryPay(_paidCellsPrices[priceIndex]))
            {
                ...
                GameObject emptyCell = await _inventoryFactory.CreateWindowEmptyCell();
        ...
        private async UniTask TrySetupPaidCell(bool createCell = false)
        {
            ...
            if (index < _paidCellsPrices.Length)
            {
                if (createCell)
                {
                    _paidCell = await _inventoryFactory.CreateWindowPaidCell();
                    _paidCell.BuyCellEvent += OnBuyCellClick;
        ...
        private async UniTask CreateEmptyCell(int siblingIndex = -1)
        {
            GameObject cell = await _inventoryFactory.CreateWindowEmptyCell();
