...
namespace MergeIt.Game.Services.Saves
{
    public class GameSaveEveryIntervalService : IGameSaveService, IInitializable, IDisposable, IUpdatable, IMonoApplicationQuitHandler
    {
        ...
        private async UniTask SaveLevel()
        {
            ...
            var values =  _fieldLogicModel.FieldElements.Values;
            var keys = _fieldLogicModel.FieldElements.Keys;
            for(int i=values.Count - 1; i>0; i--)
            {
                var fieldElement = values.ElementAt(i);
                if (fieldElement == null)
                {
                    var k = keys.ElementAt(i);
                    _fieldLogicModel.FieldElements.Remove(k);
                    continue;
                }

                FieldElementData savedElement = _configProcessor.ConvertToFieldElementData(fieldElement);
