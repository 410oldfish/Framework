...
namespace TEngine
{
    ...
    internal sealed partial class ResourceModule : Module, IResourceModule
    {
        ...
        public async UniTask<T[]> LoadAssetAsyncByTag<T>(string tag, CancellationToken cancellationToken = default, string packageName = "") where T : UnityEngine.Object
        {
            if (string.IsNullOrEmpty(tag))
            {
                throw new GameFrameworkException("Asset tag is invalid.");
            }

            AssetInfo[] assetInfos;
            if (string.IsNullOrEmpty(packageName))
            {
                assetInfos = YooAssets.GetAssetInfos(tag);
            }
            else
            {
                var package = YooAssets.GetPackage(packageName);
                assetInfos = package.GetAssetInfos(tag);
            }
            
            T[] ret = new T[assetInfos.Length];
            int index = 0;
            foreach (var assetInfo in assetInfos)
            {
                ret[index++] = await LoadAssetAsync<T>(assetInfo.Address, cancellationToken, packageName);
            }
