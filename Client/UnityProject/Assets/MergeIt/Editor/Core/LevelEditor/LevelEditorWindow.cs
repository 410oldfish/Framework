...
using MergeIt.Editor.MergeIt.Editor.Core;
using UnityEditor;
...
namespace MergeIt.Editor.LevelEditor
{
    public class LevelEditorWindow : EditorWindow
    {
        ...
        private IntegerField _SandCount;
        ...
        private Texture _SandFullTexture;
        private Texture _SandHalfTexture;
        ...
        public void CreateGUI()
        {
            ...
            _SandFullTexture = AssetDatabase.LoadAssetAtPath<Texture>("Assets/AssetRaw/UIRaw/Raw/MergeElement/img_SandFull_1.png");
            _SandHalfTexture = AssetDatabase.LoadAssetAtPath<Texture>("Assets/AssetRaw/UIRaw/Raw/MergeElement/img_SandHalf.png");
            ...
            _SandCount = _itemFrameSettings.Q<IntegerField>("SandCount");
            _SandCount.RegisterValueChangedCallback(OnSandCountChanged);
        ...
        public void ClearCell(GridPoint point)
        {
            ...
            ChangeVisualLock(false, _selectedToggle, 0);
        }
        ...
        private void OnDestroy()
        {
            ...
            _SandCount.UnregisterValueChangedCallback(OnSandCountChanged);
        ...
        private void OnSandCountChanged(ChangeEvent<int> evt)
        {
            SetSandCount(evt.newValue);
        }
        ...
        private void ClearField()
        {
            foreach (KeyValuePair<GridPoint, LevelElementData> fieldElement in _fieldElements)
            {
                ...
                ChangeVisualLock(false, toggle, 0);
        ...
        private void SetSandCount(int sandCount)
        {
            EvolutionSelection.SetSandCell(sandCount);
        }

        private void ChangeVisualLock(bool isLocked, Toggle toggle, int sandCount)
        {
            ...
            var sand = toggle.Q<Image>("Sand");
            sandCount = Math.Clamp(sandCount, 0, 2);
            if (sandCount == 0)
            {
                if (sand != null)
                {
            ...
            else
            {
                ...
                sand = CreateSand(sandCount);
                sand.tintColor = new Color(1, 1, 1, 0.6f);
                toggle.Add(sand);
            }
        }
        ...
        private void DrawElementCell(LevelElementData elementData, Toggle toggle)
        {
            ...
            ChangeVisualLock(elementData.IsBlocked, toggle, elementData.SandCount);
        }
        ...
        private Image CreateSand(int count)
        {
            var sand = new Image
            {
                name = "Sand",
                image = count == 2?_SandFullTexture:_SandHalfTexture,
                style =
                {
                    left = StylesConstants.Length0,
                    right = StylesConstants.Length0,
        ...
        private void SetItemData()
        {
            ...
            _SandCount.SetValueWithoutNotify(EvolutionSelection.ElementData.SandCount);
        }
